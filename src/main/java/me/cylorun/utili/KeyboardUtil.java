...
public final class KeyboardUtil { // stolen from julti
    ...
    public static void copyToClipboard(String string) {
        try {
            ...
            Logger.log(Logger.WARNING,"failed to copy to clipboard");
        }
    }
    ...
    private static Pair<Integer, Boolean> virtualKeyToScanCode(int virtualKey) {
        int scanCode = me.cylorun.win32.User32.INSTANCE.MapVirtualKeyA(new WinDef.UINT(virtualKey), new WinDef.UINT(0)).intValue();
    ...
    public static void sendKeyDownToHwnd(HWND hwnd, int virtualKey) {
        me.cylorun.win32.User32.INSTANCE.PostMessageA(hwnd, new WinDef.UINT(256), new WinDef.WPARAM(virtualKey), createLParamKeyDown(virtualKey));
    }
    ...
    public static void sendKeyUpToHwnd(HWND hwnd, int virtualKey) {
        me.cylorun.win32.User32.INSTANCE.PostMessageA(hwnd, new WinDef.UINT(257), new WinDef.WPARAM(virtualKey), createLParamKeyUp(virtualKey));
    }
    ...
    public static void sendCharToHwnd(HWND hwnd, int character) {
        me.cylorun.win32.User32.INSTANCE.PostMessageA(hwnd, new WinDef.UINT(WinUser.WM_CHAR), new WinDef.WPARAM(character), new WinDef.LPARAM(0));
    }
    public static void sendStringToHwnd(HWND hwnd, String string){
        for (int i = 0; i < string.length(); i++) {
            sendCharToHwnd(hwnd, string.charAt(i));
            try {
                Thread.sleep(50);
